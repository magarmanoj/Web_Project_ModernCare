...
// add projecten
if (!$stmtproject = $conn->prepare("insert into NoodVerzoek (VerzoekID, PatientID, OngevalTypeID, Status) values (?,?,?,?)")) {
    die('{"error":"Prepared Statement failed on prepare","errNo":"' . json_encode($conn->errno) . '","mysqlError":"' . json_encode($conn->error) . '","status":"fail"}');
}

if (!$stmtproject->bind_param("iiis", $postvars['VerzoekID'], $postvars['PatientID'], $postvars['OngevalTypeID'], $postvars['Status'])) {
